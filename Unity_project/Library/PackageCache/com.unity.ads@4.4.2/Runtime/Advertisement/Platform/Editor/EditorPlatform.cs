...
namespace UnityEngine.Advertisements.Platform.Editor
{
    internal class EditorPlatform : INativePlatform
    {
        ...
        public void Initialize(string gameId, bool testMode, IUnityAdsInitializationListener initializationListener)
        {
            ...
            request.BeginGetResponse(result =>
            {
                ...
                catch  (Exception exception)
                {
                    m_Platform.UnityLifecycleManager.Post(() =>
                    {
                        string errorMessage = gameId == null ? "Invalid gameId: null" : $"Invalid configuration request for gameId: {gameId}";
                        Debug.LogError(errorMessage);
                        initializationListener?.OnInitializationFailed(UnityAdsInitializationError.INTERNAL_ERROR, errorMessage);
                    });
                }
            }, new object());
        }
        ...
        {
            return AssemblyInfoUtilities.GetCurrentAssemblyInfoVersion();
        }
